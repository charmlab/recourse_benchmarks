...
def getDistanceBetweenSamples(sample_1, sample_2, norm_type, dataset_obj):
    ...
    assert sorted(sample_1.keys()) == sorted(
        dataset_obj.getInputOutputAttributeNames("kurz")
    )
    ...
    for attr_name_kurz in np.intersect1d(mutable_attributes, non_hot_attributes):
        normalized_absolute_distances.append(
            ...
            abs(sample_1[attr_name_kurz] - sample_2[attr_name_kurz])
            / (
    ...
    for attr_name_kurz in np.intersect1d(mutable_attributes, one_hot_attributes):
        if attr_name_kurz not in already_considered:
            ...
            if "cat" in dataset_obj.attributes_kurz[attr_name_kurz].attr_type:
                ...
                sub_sample_2 = [sample_2[x] for x in siblings_kurz]
                normalized_absolute_distances.append(
                    np.sum(np.abs(np.subtract(sub_sample_1, sub_sample_2)))
                    / len(sub_sample_1)
                )
            else:
    ...
    normalized_squared_distances = [
        distance**2 for distance in normalized_absolute_distances
    ]

    # 4. sum up over everything allowed...
    zero_norm_distance = (
        1
        / len(normalized_absolute_distances)
        * np.count_nonzero(normalized_absolute_distances)
    )
    one_norm_distance = (
        1 / len(normalized_absolute_distances) * sum(normalized_absolute_distances)
    )
    two_norm_distance = np.sqrt(
        1 / len(normalized_squared_distances) * sum(normalized_squared_distances)
    )  # note the sqrt(1/len) guarantees dist \in [0,1]
    infty_norm_distance = (
        1 / len(normalized_absolute_distances) * max(normalized_absolute_distances)
    )
