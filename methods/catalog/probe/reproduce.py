...
training_params_linear = {
    ...
    "compass": {"lr": 0.002, "epochs": 25, "batch_size": 128},
    "credit": {"lr": 0.002, "epochs": 50, "batch_size": 2048},
}
training_params_mlp = {
    "adult": {"lr": 0.002, "epochs": 30, "batch_size": 1024},
    "compass": {"lr": 0.002, "epochs": 25, "batch_size": 25},
    "credit": {"lr": 0.002, "epochs": 50, "batch_size": 2048},
}

training_params = {"linear": training_params_linear, "mlp": training_params_mlp}
...
def run_experiment(
    ...
    if cf_method == "probe":
        df_cfs = expect(
            model, test_factual, sigma2=sigma2, invalidation_target=invalidation_target
        )
    else:
    ...
    for i, x in df_cfs.iterrows():
        ...
        if backend == "pytorch":
            ...
            delta_M = np.mean(1 - (model.predict(X_pert).squeeze() > 0.5).astype(int))
    ...
    cost = np.mean(
        np.abs(
            test_factual.drop(columns=["prediction"]).values
            - df_cfs.drop(columns=["prediction"]).values
        ).sum(axis=1)
    )
