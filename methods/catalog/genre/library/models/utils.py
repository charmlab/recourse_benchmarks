...
class RealEmbeddingNet(nn.Module):
    ...
    def __init__(self, input_dim: int, embedding_dim: int = 32, src_dim=1):
        ...
        self.models = nn.ModuleList(
            [
                RealEmbedding(self.embedding_dim, src_dim=src_dim)
    ...
    def forward(self, x):
        ...
        embs = [
            model(x[:, i].view(-1, self.src_dim)).view(-1, 1, self.embedding_dim)
            for i, model in enumerate(self.models[:seq_len])
        ]
...
class HetLinearLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        outs = [
            layer(x[:, i]).unsqueeze(1) for i, layer in enumerate(self.layers[:seq_len])
        ]
...
class sklearnEpsAdjustedClf:
    ...
    def __init__(self, eps, prob_func, density_func, prior, denorm=None):
        ...
        if eps == 0:
            ...
            self.frac_func = np.vectorize(
                lambda x1, x2: eps / (eps + density_func(x1, x2))
            )
