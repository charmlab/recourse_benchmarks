...
def _train_ann(
    ...
    CLF_FOLDER = utils.get_ann_folder(
        HIDDEN_LAYER_SIZE,
        DATASET_STR,
        LABEL_SRC,
        CALIBRATED,
    ...
    if os.path.exists(STATE_PATH) and not FORCE_RETRAIN:
        ...
        train_y, train_X, test_y, test_X = dutils.load_dataset(
            DATASET_STR, cust_labels_path=LABEL_PATH, ret_tensor=True, min_max=MIN_MAX
        )
        ...
        for epoch in range(1, N_EPOCHS + 1):
            train_loss = ann.train_epoch(
                ann_model, optimizer, train_loader, epoch, DEVICE
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--calibrated",
        action="store_true",
        help="flag to specify if model should be calibrated",
    )
    parser.add_argument(
        "--epochs", type=int, default=100, help="number of epochs to train"
    )
    parser.add_argument(
        "--batch", type=int, default=2048, help="batch size for training"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--to",
        action="store_false",
        help="flag to specify if lsrc should be only trained on train split only",
    )
    parser.add_argument(
        "--std",
        action="store_false",
        help="flag to specify if std normalisation should be uses",
    )
    ...
    if LABEL_SRC == "gold":
        assert not (
            TRAIN_TEST_LABEL_SRC
        ), "whether label src is trained on train test or not, applicable when src is not gold"
