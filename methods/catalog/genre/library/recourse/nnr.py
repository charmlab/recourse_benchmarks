...
class NNR:
    ...
    def __call__(self, xf_r, ystar, DEVICE):
        ...
        nnr_loader = torch.utils.data.DataLoader(
            nnr_dataset, batch_size=self.batch_size, shuffle=False
        )
        xcf_nnr = []
        for batch in nnr_loader:
            bxf_r = batch[0].to(DEVICE)
            # TODO: only calculates p=1 for cost, extend to general cost norm
            dist_matrix_nnr = utils.get_pair_dist(
                bxf_r, train_positive_examples, p=self.p
            )
