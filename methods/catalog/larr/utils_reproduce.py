...
class Dataset(ABC):
    ...
    def get_data(self, fold: int, shift: bool = False):
        ...
        if shift:
            return self.split_data(self.X, self.y, fold), self.split_data(
                self.X_shift, self.y_shift, fold
            )
        else:
...
class GermanDataset(Dataset):
    def __init__(self, seed=0, n_folds=5):
        ...
        self.X_shift, self.y_shift = self.create(
            "methods/catalog/larr/corrected_german.csv"
        )
        self.name = "german"
    ...
    def create(self, filepath):
        ...
        cat_features, num_features = ["personal_status_sex"], [
            "duration",
            "amount",
            "age",
        ]
        ...
        df = df.drop(
            columns=[
                c for c in list(df) if c not in num_features + cat_features + [target]
            ]
        )
