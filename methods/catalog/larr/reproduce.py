...
def test_run_experiment():
    ...
    for seed in range(5):
        ...
        lar_recourse.choose_lambda(recourse_needed_X_train, base_model.predict, X_train.values)

        n = 5

        for i in tqdm.trange(n, desc=f'Evaluating recourse | alpha={lar_recourse.alpha}; lambda={lar_recourse.lamb}', colour='#0091ff'):
            x_0 = recourse_needed_X_test[i]

            J = RecourseCost(x_0, lar_recourse.lamb)

            np.random.seed(i)
            weights_0, bias_0 = lar_recourse.lime_explanation(base_model.predict, X_train, x_0)
    ...
    print(f"Avge Robustness: {avge_robustness} and Avge Consistency: {avge_consistency}") # we should have around 25 
