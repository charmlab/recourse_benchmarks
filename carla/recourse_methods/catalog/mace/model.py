...
from carla.recourse_methods.catalog.mace.library import generateExplanations
import os
...
class MACE(RecourseMethod):
  ...
  def __init__(self, mlmodel: MLModelCatalog, hyperparameters: Optional[Dict] = None):
  ...
  def get_counterfactuals(self, factuals: pd.DataFrame):
    ...
    dataset_obj = loadData.loadDataset(dataset_string, return_one_hot = True, load_from_cache = False, debug_flag = False)
    ...
    for norm_type_string in self._norm_type:
      for factual_sample_index, factual_sample in list(iterate_over_data_dict.items()):
    ...
    recourse_counterfactuals = pd.concat([(pd.DataFrame(counterfactuals)).drop(['y'] if not pd.DataFrame(counterfactuals).empty else {}, axis=1) for counterfactuals in recourse_counterfactuals], ignore_index=True)
