...
def load_mortgage_data():
    U_0 = (
        np.random.poisson(salary_lambda, (n, 1)) + np.random.normal(0, 1, (n, 1))
    ) * salary_multiplier
    ...
    y = (
        np.sign(np.sign(np.dot(X, w) + b) + 1e-6) + 1
    ) / 2  # add 1e-3 to prevent label 0.5
    ...
    data_frame_non_hot = pd.DataFrame(
        np.concatenate(
            (
                np.concatenate(
                    (y_train, X_train), axis=1
                ),  # importantly, labels have to go first, else Dataset.__init__ messes up kurz column names
                np.concatenate(
                    (y_test, X_test), axis=1
                ),  # importantly, labels have to go first, else Dataset.__init__ messes up kurz column names
            ),
            axis=0,
        ),
