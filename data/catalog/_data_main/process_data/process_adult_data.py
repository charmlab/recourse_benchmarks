...
def load_adult_data(load_data_size=None):
    ...
    for f in data_files:
        ...
        for line in open(f):
            ...
            line = line.split(", ")
            if (
                len(line) != 15 or "?" in line
            ):  # if a line has missing attributes, ignore it
                continue
    ...
    for attr_name in attrs_for_classification:
        ...
        if (
            attr_name in int_attrs or attr_name == "native_country"
...
def load_adult_data_new():
    ...
    processed_df.loc[
        df["marital_status"] == "Married-spouse-absent", "MaritalStatus"
    ] = 4
