...
def load_credit_data():
    ...
    raw_data_file = os.path.join(
        os.path.dirname(__file__), "..", "raw_data", "credit_raw.csv"
    )
    ...
    raw_df[monetary_features] = (
        raw_df[monetary_features]
        .applymap(lambda x: x / NTD_to_USD)
        .round(-1)
        .astype(int)
    )

    # outcome variable in first column
    processed_df["NoDefaultNextMonth (label)"] = (
        1 - raw_df["default payment next month (label)"]
    )
    ...
    processed_df.loc[
        raw_df.MARRIAGE == 1, "isMarried"
    ] = True  # married (use T/F, but not 1/0, so that some values become NAN and can be dropped later!)
    processed_df.loc[
        raw_df.MARRIAGE == 2, "isMarried"
    ] = False  # single (use T/F, but not 1/0, so that some values become NAN and can be dropped later!)
