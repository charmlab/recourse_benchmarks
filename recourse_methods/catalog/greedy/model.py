...
class Greedy(RecourseMethod):
    ...
    def cross_entropy_loss(self, predictions, target_class):
        ...
        return tf.nn.sigmoid_cross_entropy_with_logits(labels=target, logits=prediction_val)
        ...
        for index, row in factuals.iterrows():
            ...
            with tf.Session() as sess:
                ...
                for i in range(self.max_iter):
                    ...
                    classification_loss = self.cross_entropy_loss(predictions[0, 1], self.target_class)
                    loss = self.lambda_param * distance_loss + classification_loss

                    gradients = tf.gradients(loss, [x])[0]  # Use list to get gradient for tf.Variable
                    ...
                    if (current_prediction >= 0.5 and self.target_class == 1) or (current_prediction < 0.5 and self.target_class == 0):
                        break
