    ...
    def recurse(node, depth):
        ...
        if tree_.feature[node] != _tree.TREE_UNDEFINED:
            ...
            if return_value == "class_idx_max":
                ...
                prob_array = list(
                    np.divide(tree_.value[node][0], np.sum(tree_.value[node][0]))
                )
...
def tree2c(tree, feature_names, return_value="class_idx_max", tree_idx=""):
    ...
    lines = list()
    lines.append(
        "proc predict_tree{}({} : int) : int = {{".format(
            tree_idx, " : int, ".join(feature_names)
        )
    )
    lines.append("")
    ...
    def recurse(node):
        if tree_.feature[node] != _tree.TREE_UNDEFINED:
            ...
            return Or(
                And(
                    LE(
                        ToReal(model_symbols["counterfactual"][name]["symbol"]),
                        Real(threshold),
                    ),
            ...
            if return_value == "class_idx_max":
                ...
                return And(
                    EqualsOrIff(
                        model_symbols["aux"][f"p0{tree_idx}"]["symbol"],
                        Real(float(prob_array[0])),
                    ),
                    EqualsOrIff(
                        model_symbols["aux"][f"p1{tree_idx}"]["symbol"],
                        Real(float(prob_array[1])),
                    ),
                )
...
def forest2py(forest, feature_names):
    ...
    for tree_idx in range(len(forest.estimators_)):
        ...
        lines.append(
            tree2py(
                tree, feature_names, return_value="class_prob_array", tree_idx=tree_idx
            )
        )
        lines.append("")
        lines.append("")
    ...
    for tree_idx in range(len(forest.estimators_)):
        lines.append("")
        lines.append(
            "\tsum_prob[0] = sum_prob[0] + predict_tree{}({})[0]".format(
                tree_idx, ", ".join(feature_names)
            )
        )
        lines.append(
            "\tsum_prob[1] = sum_prob[1] + predict_tree{}({})[1]".format(
                tree_idx, ", ".join(feature_names)
            )
        )
    lines.append("")
...
def forest2c(forest, feature_names):
    ...
    for tree_idx in range(len(forest.estimators_)):
        ...
        lines.append("")
    lines.append(
        "proc predict_forest({} : int) : int = {{".format(", ".join(feature_names))
    )
    lines.append("")
...
def forest2formula(forest, model_symbols):
    model_symbols["aux"] = {}
    for tree_idx in range(len(forest.estimators_)):
        model_symbols["aux"][f"p0{tree_idx}"] = {
            "symbol": Symbol(f"p0{tree_idx}", REAL)
        }
        model_symbols["aux"][f"p1{tree_idx}"] = {
            "symbol": Symbol(f"p1{tree_idx}", REAL)
        }

    tree_formulas = And(
        [
            tree2formula(
                forest.estimators_[tree_idx],
                model_symbols,
                return_value="class_prob_array",
                tree_idx=tree_idx,
            )
            for tree_idx in range(len(forest.estimators_))
        ]
    ...
    return And(tree_formulas, output_formula)
...
def lr2py(model, feature_names):
    ...
    lines.append(
        "\tw = [{}]".format(", ".join(["{}".format(w) for w in model.coef_[0]]))
    )
...
def lr2c(model, feature_names):
    lines = list()
    lines.append(
        "proc predict_lr({} : int) : int = {{".format(" : int, ".join(feature_names))
    )
    lines.append("")
    lines.append(
        "\tvar {}, score : int;".format(
            ", ".join(["w" + str(i) for i in range(len(feature_names))])
        )
    )
    lines.append("\tvar output : int;")
    lines.append("")
    ...
    for i in range(len(feature_names)):
        lines.append(
            "\tscore{} = score{} + ({} * w{});".format(i + 1, i, feature_names[i], i)
        )
    lines.append("")
            ...
            Plus(
                ...
                Plus(
                    [
                        Times(
                            ToReal(
                                model_symbols["counterfactual"][symbol_key]["symbol"]
                            ),
                            Real(float(model.coef_[0][idx])),
                        )
                        for idx, symbol_key in enumerate(
...
def mlp2c(model, feature_names):
    ...
    lines = []
    lines.append(
        "proc predict_mlp({} : int) : int = {{".format(" : int, ".join(feature_names))
    )
            ...
            for curr_layer_feature_idx in range(interlayer_weight_matrix.shape[1]):
                lines.append(
                    "\tw_{}_{}_{} = {};".format(
                        ...
                        interlayer_weight_matrix[
                            prev_layer_feature_idx, curr_layer_feature_idx
                        ],
                    )
                )
    ...
    for feature_idx in range(layer_widths[0]):
        lines.append(
            "\tf_{}_{} = {};".format(0, feature_idx, feature_names[feature_idx])
        )
    ...
    for layer_idx in range(1, len(layer_widths)):
        ...
        for curr_layer_feature_idx in range(layer_widths[layer_idx]):
            ...
            for prev_layer_feature_idx in range(layer_widths[layer_idx - 1]):
                prev_layer_feature = "f_{}_{}".format(
            ...
            lines.append(
                "\t{} = {} + {};".format(
                    curr_layer_feature, curr_layer_feature, " + ".join(tmp_strings)
                )
            )
            #
    ...
    lines.append("")
    lines.append(
        "\toutput = {};".format(curr_layer_feature)
    )  # TODO: if > 1, return TRUE, else FALSE
    lines.append("\treturn output;")
    lines.append("")
    lines.append("}")
    lines.append("")
...
def mlp2formula(model, model_symbols):
    ...
    for layer_idx in range(1, len(layer_widths)):
        for feature_idx in range(layer_widths[layer_idx]):
            ...
            model_symbols["aux"][feature_string_1] = {
                "symbol": Symbol(feature_string_1, REAL)
            }
            model_symbols["aux"][feature_string_2] = {
                "symbol": Symbol(feature_string_2, REAL)
            }
    ...
    for layer_idx in range(1, len(layer_widths)):
        ...
        for curr_layer_feature_idx in range(layer_widths[layer_idx]):
            curr_layer_feature_string_1 = "f_{}_{}_pre_nonlin".format(
            ...
            inputs_to_curr_layer_feature.append(
                Real(float(model.intercepts_[layer_idx - 1][curr_layer_feature_idx]))
            )

            for prev_layer_feature_idx in range(layer_widths[layer_idx - 1]):
                if layer_idx == 1 and prev_layer_feature_idx < len(
                    list(model_symbols["counterfactual"].keys())
                ):
                    ...
                    inputs_to_curr_layer_feature.append(
                        Times(
                            ToReal(
                                model_symbols["counterfactual"][
                                    input_layer_feature_string
                                ]["symbol"]
                            ),
                            Real(
                                float(
