...
def getCounterfactualFormula(model_symbols, factual_sample):
    return EqualsOrIff(
        model_symbols["output"]["y"]["symbol"], Not(factual_sample["y"])
    )  # meaning we want the decision to be flipped.


def getDistanceFormula(
    model_symbols,
    dataset_obj,
    factual_sample,
    norm_type,
            ...
            Div(
                ToReal(
                    getAbsoluteDifference(
                        model_symbols[variable_to_compute_distance_on][attr_name_kurz][
                            "symbol"
                        ],
        ...
        normalized_squared_distances.append(
            Pow(
                Div(
                    ToReal(
                        Minus(
                            model_symbols[variable_to_compute_distance_on][
                                attr_name_kurz
                            ]["symbol"],
                            factual_sample[attr_name_kurz],
                        )
                            ...
                            ToReal(
                                Minus(
                                    Plus(
                                        [
                                            model_symbols[
                                                variable_to_compute_distance_on
                                            ][attr_name_kurz]["symbol"]
                                            for attr_name_kurz in siblings_kurz
                                        ]
                                    ),
                                    Plus(
    ...
    if norm_type == "zero_norm":
        distance_formula = LE(
            Times(
                ...
                Plus(
                    [
                        Ite(Equals(elem, Real(0)), Real(0), Real(1))
        ...
        distance_formula = LE(
            ...
            Pow(Real(norm_threshold), Real(2)),
        )
...
def getPlausibilityFormula(model_symbols, dataset_obj, factual_sample, approach_string):
        ...
        [
            And(
                GE(
                    ...
                    model_symbols["counterfactual"][attr_name_kurz]["lower_bound"],
                ),
                LE(
                    model_symbols["counterfactual"][attr_name_kurz]["symbol"],
                    model_symbols["counterfactual"][attr_name_kurz]["upper_bound"],
                ),
            )
                    ...
                    EqualsOrIff(
                        Plus(
                            [
                                ...
                                for attr_name_kurz in dict_of_siblings_kurz["cat"][
                                    parent_name_kurz
                                ]
                            ]
                        ),
                ...
                And(
                    [
                        GE(
                            ToReal(
                                model_symbols["counterfactual"][
                                    dict_of_siblings_kurz["ord"][parent_name_kurz][
                                        symbol_idx
                                    ]
                                ]["symbol"]
                            ),
                        ...
                        for symbol_idx in range(
                            len(dict_of_siblings_kurz["ord"][parent_name_kurz]) - 1
                        )  # already sorted
                    ]
                ),
                And(
    ...
    if "mace" in approach_string:
        ...
        causal_consistency = getCausalConsistencyConstraints(
            model_symbols, dataset_obj, factual_sample
        )
...
def getDiversityFormulaUpdate(model):
    return Not(
        And(
            [
                EqualsOrIff(symbol_key, symbol_value)
...
def findClosestCounterfactualSample(
    model_trained,
    model_symbols,
    dataset_obj,
    factual_sample,
    ...
    def assertPrediction(dict_sample, model_trained, dataset_obj):
        ...
        assert (
            sklearn_prediction == pysmt_prediction
        ), "Pysmt prediction does not match sklearn prediction."
        assert (
            sklearn_prediction != factual_prediction
        ), "Counterfactual and factual samples have the same prediction."
    ...
    counterfactual_formula = getCounterfactualFormula(
        model_symbols, factual_pysmt_sample
    )
    plausibility_formula = getPlausibilityFormula(
        model_symbols, dataset_obj, factual_pysmt_sample, approach_string
    )
            ...
            if solved:  # joint formula is satisfiable
                ...
                if "mace" in approach_string:
                    norm_upper_bound = float(
                        counterfactual_distance + epsilon / 100
                    )  # not float64
                elif "mint" in approach_string:
                    norm_upper_bound = float(
                        interventional_distance + epsilon / 100
                    )  # not float64
                curr_norm_threshold = getCenterNormThresholdInRange(
                    norm_lower_bound, norm_upper_bound
                )
    ...
    closest_interventional_sample = sorted(
        counterfactuals, key=lambda x: x["interventional_distance"]
    )[0]
...
def getPrettyStringForSampleDictionary(sample, dataset_obj):
    ...
    for key, value in sorted(
        key_value_pairs_with_x_in_key.items(), key=lambda x: int(x[0][1:].split("_")[0])
    ):
...
def getDictSampleFromPySMTSample(pysmt_sample, dataset_obj):
    ...
    for attr_name_kurz in dataset_obj.getInputOutputAttributeNames("kurz"):
        ...
        try:
            if attr_name_kurz not in dataset_obj.getInputAttributeNames("kurz"):
                dict_sample[attr_name_kurz] = bool(
                    str(pysmt_sample[attr_name_kurz]) == "True"
                )
            elif attr_obj.attr_type == "numeric-real":
                dict_sample[attr_name_kurz] = float(
                    eval(str(pysmt_sample[attr_name_kurz]))
                )
            else:  # refer to loadData.VALID_ATTRIBUTE_TYPES
...
def genExp(
    ...
    (
        all_counterfactuals,
        closest_counterfactual_sample,
        closest_interventional_sample,
    ) = findClosestCounterfactualSample(
        model_trained,
        model_symbols,
        dataset_obj,
        factual_sample,
    ...
    if "mace" in approach_string:
        ...
        for attr_name_kurz in dataset_obj.getInputAttributeNames("kurz"):
            if (
                factual_sample[attr_name_kurz]
                != closest_interventional_sample["interventional_sample"][
                    attr_name_kurz
                ]
            ):
                action_set[attr_name_kurz] = closest_interventional_sample[
                    "interventional_sample"
                ][attr_name_kurz]
        return {
            "fac_sample": factual_sample,
