...
methods_list = [
    ...
    "revise",
    "wachter",
]  # List of Available Recourse Methods for Radar Chart
l_norms_list = [
    "L0_distance",
    "L1_distance",
    "L2_distance",
    "Linf_distance",
]  # List of Different L Norms for Radar Chart
...
def generate_best_methods(dataset, ml_model, l_norm):
    ...
    for col in range(len(methods_values[0])):
        ...
        col_values[np.isnan(col_values)] = (
            np.nanmax(col_values) if col != 1 else 0
        )  # Convert nan to the max value for similarity and runtime or 0 for coverage
        col_values = (
            1 - (col_values / np.nanmax(col_values)) if col != 1 else (col_values)
        )
...
modal = html.Div(
    [
        ...
        dbc.Modal(
            [
                dbc.ModalHeader(
                    dbc.ModalTitle("CARLA Benchmark Command"),
...
app.layout = html.Div(
    [
        ...
        html.Div(
            [
                dcc.Graph(figure={}, id=f"radar-graph-{method}")
                for method in l_norms_list
            ],
            style={
                "display": "grid",
...
def update_graph(dataset, ml_model, top_val_constraints):
    ...
    for norm_label, norm_value in l_norm_to_category_column_dict.items():
        ...
        data = [
            go.Scatterpolar(r=row, theta=categories, fill="toself", name=label)
            for label, row in methods_info.items()
        ]
        ...
        curr_fig = go.Figure(
            ...
            layout=go.Layout(
                title=go.layout.Title(
                    text=f"Chart for {norm_label}", xanchor="center", x=0.5
                ),
                polar=dict(
                    bgcolor="white",
                    angularaxis=dict(gridwidth=1, gridcolor="black"),
                    radialaxis=dict(
                        linewidth=1,
                        gridcolor="black",
                        linecolor="black",
        ...
        trace_area_df = pd.DataFrame(
            {"Trace": trace_area.index, "Area": trace_area.values}
        )
        ...
        curr_fig.for_each_trace(
            lambda t: t.update(name=f"{t.name}, Area: {trace_area.loc[t.name]:.5f}")
        )
